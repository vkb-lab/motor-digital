from pathlib import Path
import sys
import json

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from k_atlas.product_factory.product_export_zip_writer_gate import (
    refresh_product_export_zip_writer_gate,
    get_latest_product_export_zip_writer_report,
)

if __name__ == "__main__":
    result = refresh_product_export_zip_writer_gate()
    latest = get_latest_product_export_zip_writer_report()

    print(json.dumps({
        "status": "PHASE64_PRODUCT_EXPORT_ZIP_WRITER_GATE_COMPLETED",
        "result": result,
        "latest_status": latest.get("status"),
        "products_count": result.get("products_count", 0),
        "ready_for_zip_count": result.get("ready_for_zip_count", 0),
        "blocked_count": result.get("blocked_count", 0),
        "zip_creation_allowed_now": False,
        "zip_created": False,
        "deploy_allowed": False,
        "paid_ai_allowed": False,
        "instagram_publish_allowed": False,
        "real_action_executed": False,
        "paid_ai_call_executed": False,
        "instagram_publish_executed": False
    }, ensure_ascii=False, indent=2))